...
void send_vote(RING_ELECTION_VOTE *vote)
{
    ...
    // int sock_fd = connect_socket(next_node_election_addr, next_node.port + ELECTION_PORT_OFFSET);
}
...
void *run_election_server(void *data)
{
    ...
    while (TRUE)
    {
        ...
        participant_conn->port = election_participant_addr.sin_port;
